...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			bufferedImage = ImageIO.read(new File("words.png"));
		} catch (IOException e) {
		}
		...
		convertToBinary();		
		System.out.println("No of words:"+getConnectedComponents());
	...
	public static void convertToBinary() throws IOException{
		...
		for(int i=-1; i<width+1; i++){
			for(int j=-1; j<height+1; j++){
				...
				if(c.getRed()>40 && c.getBlue()>100 ||c.getGreen()>100){
					red=0;
					blue=0;
					green=0;
				}else{
		...
		for( int i=0; i<width+2 ; i++){
			for (int j=0; j<height+2; j++){
					...
					if(c.getRed()==255){
						...
						components++;							
						cropImage(components+".jpg", minX, minY, maxX, maxY);												
					}											
			}
		}
		return components;
